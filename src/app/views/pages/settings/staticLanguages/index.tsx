...
import {LanguageKeysArray, UserRoleId} from "../../../../../constants";
...
class PageSettingsStaticLanguages extends Component<PageProps, PageState> {
    ...
    setPageTitle() {
        this.props.setBreadCrumb([this.props.router.t("settings"), this.props.router.t("staticLanguages")])
    }
        ...
        const StaticLanguage = (staticLanguageProps: SettingStaticLanguageDocument, staticLanguageIndex: number) => {
            return (
                <div className="col-md-12 mt-4">
                    <ThemeFieldSet
                        legend={`${this.props.router.t("staticLanguages")} (#${staticLanguageProps.langKey})`}
                        legendElement={
                    ...
                    >
                        <div className="row">
                            <div className="col-md-12 mt-4">
                                <ThemeFormType
                                    type="text"
                                    title={this.props.router.t(staticLanguageProps.langKey)}
        ...
        const NewStaticLanguage = (staticLanguageProps: SettingStaticLanguageDocument, staticLanguageIndex: number) => {
            return (
                <div className="col-md-12 mt-3">
                    <ThemeFieldSet legend={this.props.router.t("newStaticLanguage")}>
                        <div className="row mt-3">
                            ...
                            <div className="col-md-12 mt-3">
                                <button type={"button"} className="btn btn-gradient-success btn-lg"
                                        onClick={() => this.StaticLanguagesEvents.onAccept(staticLanguageProps._id || "")}>{this.props.router.t("okay")}</button>
                                <button type={"button"} className="btn btn-gradient-danger btn-lg"
                                        onClick={() => this.StaticLanguagesEvents.onDelete(this.state.newStaticLanguages, staticLanguageIndex)}>{this.props.router.t("delete")}</button>
                            </div>
                        </div>
                    </ThemeFieldSet>
                </div>
            ...
            <div className="row">
                {
                    this.props.getSessionData.roleId == UserRoleId.SuperAdmin
                        ? <div className="col-md-7">
                            <button type={"button"} className="btn btn-gradient-success btn-lg"
                                    onClick={() => this.StaticLanguagesEvents.onCreate()}>+ {this.props.router.t("newStaticLanguage")}
                            </button>
                        </div> : null
                }

  ...
  get sidebarData (): {Paths: Array<SideBarPath>} {
    return {
      Paths: [
        ...
        {path: pageRoutes.gallery.path(), icon: `image-multiple`, title: this.props.router.t("gallery"), state: `gallery`, subPaths: [
        ...
        {path: pageRoutes.themeContent.path(), icon: `shape`, title: this.props.router.t("themeContents"), state: `themeContents`, subPaths: [
            {path: pageRoutes.post.path(PostTypeId.Blog), title: this.props.router.t("blogs"), state: `blogs`, subPaths: [
                    {path: pageRoutes.post.add.path(), title: this.props.router.t("add")},
                    {path: pageRoutes.post.list.path(), title: this.props.router.t("list")}
              ]
            },
            {path: pageRoutes.post.path(PostTypeId.Portfolio), title: this.props.router.t("portfolios"), state: `portfolios`, subPaths: [
                    {path: pageRoutes.post.add.path(), title: this.props.router.t("add")},
                    {path: pageRoutes.post.list.path(), title: this.props.router.t("list")}
              ]
            },
            {path: pageRoutes.post.path(PostTypeId.Slider), title: this.props.router.t("sliders"), state: `sliders`, subPaths: [
                    {path: pageRoutes.post.add.path(), title: this.props.router.t("add")},
                    {path: pageRoutes.post.list.path(), title: this.props.router.t("list")}
              ]
            },
            {path: pageRoutes.post.path(PostTypeId.Reference), title: this.props.router.t("references"), state: `references`, subPaths: [
                    {path: pageRoutes.post.add.path(), title: this.props.router.t("add")},
                    {path: pageRoutes.post.list.path(), title: this.props.router.t("list")}
              ]
            }
        ...
        {path: pageRoutes.settings.path(), icon: `cog`, title: this.props.router.t("settings"), state: `settings`, subPaths: [
            {path: pageRoutes.settings.user.path(), icon: `account-multiple`, title: this.props.router.t("users"), state: `users`, subPaths: [
                  ...
                  _props.subPaths.map((item, index) => {
                      ...
                      return (
                        <li className="nav-item" key={index}>
                          {
                            (typeof item.subPaths === "undefined") ? <HasChild key={index} {...item}/> : <HasChildren key={index} {...item} />
                          }
                        </li>
                    );
                  })
            ...
            <a href="!#" className="nav-link" onClick={evt =>evt.preventDefault()}>
              ...
              <div className="nav-profile-text">
                ...
                <span className="text-secondary text-small">
                    <label className={`badge badge-gradient-${GlobalFunctions.getUserRolesClassName(getSessionData().roleId)} float-end`}>
                        {
                            GlobalFunctions.getStaticContent(UserRoleContents, "roleId", getSessionData().roleId, getSessionData().langId)
                        }
                    </label>
                </span>
              </div>
